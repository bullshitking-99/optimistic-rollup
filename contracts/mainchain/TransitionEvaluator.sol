...
contract TransitionEvaluator {
    ...
    function evaluateTransition(
        ...
        DataTypes.StorageSlot[]
            memory storageSlots = new DataTypes.StorageSlot[](
        ...
        if (transitionType == TRANSITION_TYPE_CREATE_AND_DEPOSIT) {
            DataTypes.CreateAndDepositTransition
                memory createAndDeposit = decodeCreateAndDepositTransition(
                    transition
                );

            DataTypes.AccountInfo
                memory updatedAccountInfo = applyCreateAndDepositTransition(
                    createAndDeposit,
                    storageSlots[0]
                );
            ...
            DataTypes.DepositTransition
                memory deposit = decodeDepositTransition(transition);

            DataTypes.AccountInfo
                memory updatedAccountInfo = applyDepositTransition(
                    deposit,
                    storageSlots[0]
                );
            outputs = new bytes32[](1);
            ...
            DataTypes.WithdrawTransition
                memory withdraw = decodeWithdrawTransition(transition);

            DataTypes.AccountInfo
                memory updatedAccountInfo = applyWithdrawTransition(
                    withdraw,
                    storageSlots[0]
                );
            outputs = new bytes32[](1);
            ...
            DataTypes.CreateAndTransferTransition
                memory createAndTransfer = decodeCreateAndTransferTransition(
                    transition
                );

            DataTypes.AccountInfo[2]
                memory updatedAccountInfos = applyCreateAndTransferTransition(
                    createAndTransfer,
            ...
            DataTypes.TransferTransition
                memory transfer = decodeTransferTransition(transition);

            DataTypes.AccountInfo[2]
                memory updatedAccountInfos = applyTransferTransition(
                    transfer,
                    [storageSlots[0], storageSlots[1]]
                );
            // Return the hash of both storage (leaf nodes to insert into the tree)
    ...
    function extractTransitionType(
        bytes memory _bytes
    ...
    function getTransitionStateRootAndAccessList(
    ...
    ) external pure returns (bytes32, uint256[] memory) {
        ...
        if (transitionType == TRANSITION_TYPE_CREATE_AND_DEPOSIT) {
            DataTypes.CreateAndDepositTransition
                memory transition = decodeCreateAndDepositTransition(
                    rawTransition
                );
            ...
            DataTypes.DepositTransition
                memory transition = decodeDepositTransition(rawTransition);
            stateRoot = transition.stateRoot;
            storageSlots = new uint256[](1);
            storageSlots[0] = transition.accountSlotIndex;
        } else if (transitionType == TRANSITION_TYPE_WITHDRAW) {
            DataTypes.WithdrawTransition
                memory transition = decodeWithdrawTransition(rawTransition);
            stateRoot = transition.stateRoot;
            storageSlots = new uint256[](1);
            storageSlots[0] = transition.accountSlotIndex;
        } else if (transitionType == TRANSITION_TYPE_CREATE_AND_TRANSFER) {
            DataTypes.CreateAndTransferTransition
                memory transition = decodeCreateAndTransferTransition(
                    rawTransition
                );
            stateRoot = transition.stateRoot;
            ...
            DataTypes.TransferTransition
                memory transition = decodeTransferTransition(rawTransition);
            stateRoot = transition.stateRoot;
            storageSlots = new uint256[](2);
            storageSlots[0] = transition.senderSlotIndex;
            storageSlots[1] = transition.recipientSlotIndex;
    ...
    function getWithdrawTxHash(
        DataTypes.WithdrawTx memory _withdrawTx
    ...
    function getTransferTxHash(
        DataTypes.TransferTx memory _transferTx
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
    ...
    function verifyEmptyAccountInfo(
    ...
    function applyWithdrawTransition(
        ...
        uint256 oldWithdrawNonce = _storageSlot.value.withdrawNonces[
            tokenIndex
        ];
    ...
    function applyTransferTransition(
        ...
        uint256 recipientOldBalance = _storageSlots[1].value.balances[
            tokenIndex
        ];
        ...
        uint256 oldTransferNonce = _storageSlots[0].value.transferNonces[
            tokenIndex
        ];
    ...
    function decodeCreateAndDepositTransition(
        bytes memory _rawBytes
    ) internal pure returns (DataTypes.CreateAndDepositTransition memory) {
    ...
    function decodeDepositTransition(
        bytes memory _rawBytes
    ) internal pure returns (DataTypes.DepositTransition memory) {
        (
            uint256 transitionType,
            bytes32 stateRoot,
            uint256 accountSlotIndex,
    ...
    function decodeWithdrawTransition(
        bytes memory _rawBytes
    ) public pure returns (DataTypes.WithdrawTransition memory) {
        (
            uint256 transitionType,
            bytes32 stateRoot,
            uint256 accountSlotIndex,
    ...
    function decodeCreateAndTransferTransition(
        bytes memory _rawBytes
    ) internal pure returns (DataTypes.CreateAndTransferTransition memory) {
        (
            uint256 transitionType,
            bytes32 stateRoot,
    ...
    function decodeTransferTransition(
        bytes memory _rawBytes
    ) internal pure returns (DataTypes.TransferTransition memory) {
        (
            uint256 transitionType,
            bytes32 stateRoot,
            uint256 senderSlotIndex,
    ...
    function verifyWithdrawTransition(
        ...
        DataTypes.WithdrawTransition
            memory transition = decodeWithdrawTransition(_rawTransition);
        DataTypes.WithdrawTx memory withdrawTx = DataTypes.WithdrawTx(
            _account,
